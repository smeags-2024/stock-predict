...
LSTMPredictor::LSTMNetImpl::LSTMNetImpl(int input_size, int hidden_size, int num_layers, int output_size)
...
LSTMPredictor::LSTMPredictor(const std::string& symbol, const PredictorConfig& config)
    ...
    optimizer_ = std::make_unique<torch::optim::Adam>(model_->parameters(), torch::optim::AdamOptions(0.001));
}
...
PredictionResult LSTMPredictor::predict_next_day(const std::vector<MarketData>& recent_data) {
    ...
    // Take last sequence
    auto sequence = tensor_data.slice(0, tensor_data.size(0) - config_.sequence_length, tensor_data.size(0));
...
TransformerPredictor::TransformerNetImpl::TransformerNetImpl(
    int input_size, int d_model, int nhead, int num_layers, int output_size)
...
TransformerPredictor::TransformerPredictor(const std::string& symbol, const PredictorConfig& config)
    ...
    optimizer_ = std::make_unique<torch::optim::AdamW>(
        model_->parameters(), torch::optim::AdamWOptions(0.0001));
}
...
PredictionResult TransformerPredictor::predict_next_day(const std::vector<MarketData>& recent_data) {
    ...
    auto input = tensor_data.slice(0, tensor_data.size(0) - config_.sequence_length, 
                                  tensor_data.size(0)).unsqueeze(0).unsqueeze(2);
    input = input.to(device_);
...
torch::Tensor TransformerPredictor::create_positional_encoding(int seq_length, int d_model) {
    ...
    auto div_term = torch::exp(torch::arange(0, d_model, 2).to(torch::kFloat) * 
...
PredictionResult EnsemblePredictor::combine_predictions(const std::vector<PredictionResult>& predictions) {
    if (predictions.empty()) {
