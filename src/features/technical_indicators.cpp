...
TechnicalIndicators::macd(const std::vector<double>& prices, int fast_period, int slow_period, int signal_period) {
...
std::tuple<std::vector<double>, std::vector<double>, std::vector<double>>
TechnicalIndicators::bollinger_bands(const std::vector<double>& prices, int period, double std_dev) {
...
std::pair<std::vector<double>, std::vector<double>> TechnicalIndicators::stochastic(
    ...
    for (size_t i = k_period - 1; i < close.size(); ++i) {
        double highest_high = *std::max_element(high.begin() + i - k_period + 1, high.begin() + i + 1);
...
std::vector<double> TechnicalIndicators::williams_r(const std::vector<double>& high,
    ...
    for (size_t i = period - 1; i < close.size(); ++i) {
        double highest_high = *std::max_element(high.begin() + i - period + 1, high.begin() + i + 1);
