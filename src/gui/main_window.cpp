...
void MainWindow::setupPredictionTab() {
    ...
    m_predictButton->setStyleSheet("QPushButton { background-color: #4CAF50; color: white; font-weight: bold; padding: 8px; }");
    actionLayout->addWidget(m_predictButton);
    
    m_realTimeButton = new QPushButton("Start Real-Time");
    m_realTimeButton->setCheckable(true);
    m_realTimeButton->setStyleSheet("QPushButton { background-color: #2196F3; color: white; font-weight: bold; padding: 8px; }");
    ...
    m_predictionTable->setHorizontalHeaderLabels({"Day", "Date", "Price", "Return", "Confidence", "Risk"});
...
void MainWindow::setupTrainingTab() {
    ...
    m_loadDataButton->setStyleSheet("QPushButton { background-color: #2196F3; color: white; font-weight: bold; padding: 8px; }");
    ...
    m_trainButton->setStyleSheet("QPushButton { background-color: #4CAF50; color: white; font-weight: bold; padding: 10px; }");
...
void MainWindow::setupBacktestTab() {
    ...
    m_backtestButton->setStyleSheet("QPushButton { background-color: #FF9800; color: white; font-weight: bold; padding: 10px; }");
...
void MainWindow::setupPortfolioTab() {
    ...
    m_optimizationCombo->addItems({"Mean Variance", "Risk Parity", "Minimum Variance", "Maximum Sharpe"});
    ...
    m_optimizeButton->setStyleSheet("QPushButton { background-color: #9C27B0; color: white; font-weight: bold; padding: 10px; }");
...
void MainWindow::showPreferences() {
    m_centralTabs->setCurrentWidget(m_settingsTab);
}
...
void MainWindow::makePrediction() {
    ...
    QTimer::singleShot(500, [this]() {
        ...
        for (int i = 0; i < days; ++i) {
            ...
            m_predictionTable->setItem(i, 1, new QTableWidgetItem(QDate::currentDate().addDays(i + 1).toString()));
...
void MainWindow::startRealTimeMode() {
    ...
    m_realTimeButton->setStyleSheet("QPushButton { background-color: #F44336; color: white; font-weight: bold; padding: 8px; }");
...
void MainWindow::runBacktest() {
    ...
    QTimer::singleShot(3000, [this]() {
        ...
        QStringList metrics = {"Total Return", "Sharpe Ratio", "Max Drawdown", "Win Rate", "Profit Factor"};
...
void MainWindow::optimizePortfolio() {
    ...
    QTimer::singleShot(2000, [this]() {
        ...
        for (int i = 0; i < symbols.size(); ++i) {
            ...
            m_portfolioTable->setItem(i, 1, new QTableWidgetItem(formatPercentage(normalizedWeight)));
...
void MainWindow::updateModelMetrics() {
    ...
    for (size_t i = 0; i < metrics.size(); ++i) {
        m_metricsTable->setItem(static_cast<int>(i), 0, new QTableWidgetItem(QString::fromStdString(metrics[i].first)));
        m_metricsTable->setItem(static_cast<int>(i), 1, new QTableWidgetItem(QString::number(metrics[i].second, 'f', 4)));
    }
}

void MainWindow::updateRiskMetrics() {
    // Calculate and display risk metrics
    QStringList riskMetrics = {"VaR (95%)", "Expected Shortfall", "Volatility", "Sharpe Ratio", "Max Drawdown"};
...
void PredictionWorker::trainModel(const std::vector<stock_predict::MarketData>& data, 
    ...
    try {
        ...
        emit trainingComplete(success, success ? "Training completed successfully" : "Training failed");
...
void RealTimeUpdater::setUpdateInterval(int milliseconds) {
