...
std::vector<stock_predict::MarketData> generate_sample_data(const std::string& symbol, int days = 100) {
...
void demo_prediction(const std::string& symbol) {
    ...
    try {
        ...
        std::vector<stock_predict::MarketData> test_data(data.begin() + train_size - 60, data.end());
        
        std::cout << "✓ Split data: " << train_size << " training, " 
                  << (data.size() - train_size) << " testing\n";
        ...
        // Make predictions
        std::cout << "\n📈 Making Predictions:\n";
        auto recent_data = std::vector<stock_predict::MarketData>(test_data.end() - 60, test_data.end());
        ...
        for (size_t i = 0; i < multi_predictions.size(); ++i) {
            ...
            std::cout << std::setw(4) << (i + 1) << " | $"
                      << std::setw(6) << std::fixed << std::setprecision(2) << pred.price
                      << " | " << std::setw(5) << std::fixed << std::setprecision(1) 
                      << (pred.expected_return * 100) << "%"
...
void demo_risk_analysis(const std::string& symbol) {
    ...
    try {
        ...
        auto expected_shortfall = stock_predict::RiskManager::calculate_expected_shortfall(returns, 0.95);
        ...
        auto max_drawdown = stock_predict::RiskManager::calculate_max_drawdown({data[0].close, data.back().close});
        ...
        std::cout << "Expected Shortfall: " << std::fixed << std::setprecision(2) << (expected_shortfall * 100) << "%\n";
        std::cout << "Sharpe Ratio: " << std::fixed << std::setprecision(2) << sharpe_ratio << "\n";
        std::cout << "Annualized Volatility: " << std::fixed << std::setprecision(2) << (volatility * 100) << "%\n";
        std::cout << "Maximum Drawdown: " << std::fixed << std::setprecision(2) << (max_drawdown * 100) << "%\n";
        ...
        auto kelly_fraction = stock_predict::PositionSizer::kelly_criterion(win_rate, avg_win, avg_loss);
        auto fixed_fraction = stock_predict::PositionSizer::fixed_fractional(capital, 0.02, data.back().close, data.back().close * 0.95);
        auto vol_sizing = stock_predict::PositionSizer::volatility_sizing(capital, 0.15, volatility);
        
        std::cout << "Kelly Criterion: " << std::fixed << std::setprecision(1) << (kelly_fraction * 100) << "% of capital\n";
        std::cout << "Fixed Fractional (2% risk): $" << std::fixed << std::setprecision(0) << fixed_fraction << "\n";
        std::cout << "Volatility Sizing: " << std::fixed << std::setprecision(1) << (vol_sizing * 100) << "% of capital\n";
