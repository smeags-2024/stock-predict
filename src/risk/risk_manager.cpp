...
double RiskManager::calculate_sharpe_ratio(const std::vector<double>& returns, double risk_free_rate) {
    ...
    return volatility != 0.0 ? (excess_return * std::sqrt(252.0)) / (volatility * std::sqrt(252.0)) : 0.0;
}
