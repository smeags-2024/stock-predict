...
protected:
    ...
    void generateSampleData() {
        ...
        for (int i = 0; i < 252; ++i) {
            ...
            data.high = base_price * (1.0 + std::max(0.0, change) + std::abs(price_change(gen)) * 0.5);
            data.low = base_price * (1.0 + std::min(0.0, change) - std::abs(price_change(gen)) * 0.5);
...
TEST_F(TechnicalIndicatorsTest, T3_1_1_MovingAverages) {
    ...
    EXPECT_THROW(technical_indicators->calculateSMA(sample_data, sample_data.size() + 1), std::invalid_argument);
}
...
TEST_F(MarketAnalysisTest, T3_3_1_TrendAnalysis) {
    ...
    EXPECT_TRUE(trend == TrendDirection::UPTREND || 
...
TEST_F(MarketAnalysisTest, T3_3_2_VolatilityAnalysis) {
    ...
    EXPECT_TRUE(vol_regime == VolatilityRegime::LOW ||
...
TEST_F(FeatureSelectionTest, T3_4_2_FeatureImportance) {
    ...
    auto importance_scores = feature_extractor->calculateFeatureImportance(all_features, sample_data);
...
TEST_F(FeatureSelectionTest, T3_4_3_FeatureSelection) {
    ...
    auto selected_features_imp = feature_extractor->selectFeaturesByImportance(all_features, sample_data, 0.1);
