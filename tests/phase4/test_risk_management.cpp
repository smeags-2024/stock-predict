...
protected:
    ...
    void generateSamplePortfolio() {
        ...
        std::vector<std::string> symbols = {"AAPL", "GOOGL", "MSFT", "TSLA", "AMZN", "NVDA", "META", "NFLX", "SPY", "QQQ"};
        std::vector<double> weights = {0.15, 0.12, 0.10, 0.08, 0.10, 0.12, 0.08, 0.05, 0.10, 0.10};
        
        for (size_t i = 0; i < symbols.size(); ++i) {
            PortfolioPosition position;
            position.symbol = symbols[i];
            position.shares = static_cast<int>((initial_capital * weights[i]) / (100.0 + i * 10)); // Varying share prices
            position.avg_cost = 100.0 + i * 10;
            position.current_price = position.avg_cost * (0.95 + (i % 3) * 0.05); // Some gains/losses
    ...
    void generateSamplePositions() {
        ...
        for (const auto& portfolio_pos : sample_portfolio) {
            ...
            position.entry_time = std::chrono::system_clock::now() - std::chrono::hours(24 * (rand() % 30));
            position.position_type = (position.unrealized_pnl >= 0) ? PositionType::LONG : PositionType::SHORT;
...
TEST_F(RiskCalculationTest, T4_1_4_CorrelationRisk) {
    ...
    for (int asset = 0; asset < 5; ++asset) {
        ...
        for (int day = 0; day < 100; ++day) {
            double return_val = ((rand() % 201 - 100) / 10000.0) + asset * 0.0001; // Slight bias per asset
...
TEST_F(PortfolioManagementTest, T4_2_3_PerformanceMetrics) {
    ...
    double sharpe_ratio = portfolio_manager->calculateSharpeRatio(daily_returns, 0.02); // 2% risk-free rate
...
TEST_F(PortfolioManagementTest, T4_2_5_PortfolioOptimization) {
    ...
    auto optimal_weights = portfolio_manager->optimizeMeanVariance(expected_returns, covariance_matrix, 0.10);
    ...
    auto max_sharpe_weights = portfolio_manager->optimizeMaxSharpe(expected_returns, covariance_matrix, 0.02);
...
TEST_F(PositionSizingTest, T4_3_1_FixedFractionalSizing) {
    ...
    // Test edge cases
    EXPECT_THROW(position_sizer->calculateFixedFractional(account_value, 0.0), std::invalid_argument);
    EXPECT_THROW(position_sizer->calculateFixedFractional(account_value, 1.1), std::invalid_argument);
...
TEST_F(PositionSizingTest, T4_3_2_KellyPosition) {
    ...
    double kelly_fraction = position_sizer->calculateKellyFraction(win_rate, avg_win, std::abs(avg_loss));
    EXPECT_GT(kelly_fraction, 0.0);
    EXPECT_LE(kelly_fraction, 1.0);
    
    // Test Kelly position size
    double account_value = 100000.0;
    double kelly_position = position_sizer->calculateKellyPosition(account_value, win_rate, avg_win, std::abs(avg_loss));
    EXPECT_GT(kelly_position, 0.0);
    EXPECT_LE(kelly_position, account_value);
    
    // Test fractional Kelly (more conservative)
    double fractional_kelly = position_sizer->calculateFractionalKelly(account_value, win_rate, avg_win, std::abs(avg_loss), 0.5);
...
TEST_F(PositionSizingTest, T4_3_3_VolatilityBasedSizing) {
    ...
    double vol_position = position_sizer->calculateVolatilityBased(account_value, target_volatility, asset_volatility);
    EXPECT_GT(vol_position, 0.0);
    EXPECT_LT(vol_position, account_value);
    
    // Higher asset volatility should result in smaller position
    double high_vol_position = position_sizer->calculateVolatilityBased(account_value, target_volatility, 0.50);
...
TEST_F(PositionSizingTest, T4_3_5_AdaptivePositionSizing) {
    ...
    double adaptive_size = position_sizer->calculateAdaptiveSize(account_value, recent_returns, base_risk);
    ...
    double winning_size = position_sizer->calculateAdaptiveSize(account_value, winning_returns, base_risk);
    ...
    double losing_size = position_sizer->calculateAdaptiveSize(account_value, losing_returns, base_risk);
...
TEST_F(StopLossManagementTest, T4_4_1_FixedStopLoss) {
    ...
    double long_stop = stop_loss_manager->calculateFixedStopLoss(entry_price, stop_percentage, PositionType::LONG);
    EXPECT_DOUBLE_EQ(long_stop, entry_price * (1.0 - stop_percentage));
    
    // Test fixed percentage stop loss for short position
    double short_stop = stop_loss_manager->calculateFixedStopLoss(entry_price, stop_percentage, PositionType::SHORT);
    EXPECT_DOUBLE_EQ(short_stop, entry_price * (1.0 + stop_percentage));
    
    // Test fixed dollar amount stop loss
    double dollar_amount = 5.0;
    double long_dollar_stop = stop_loss_manager->calculateFixedDollarStopLoss(entry_price, dollar_amount, PositionType::LONG);
    EXPECT_DOUBLE_EQ(long_dollar_stop, entry_price - dollar_amount);
    
    double short_dollar_stop = stop_loss_manager->calculateFixedDollarStopLoss(entry_price, dollar_amount, PositionType::SHORT);
...
TEST_F(StopLossManagementTest, T4_4_3_TrailingStopLoss) {
    ...
    auto trailing_stop = stop_loss_manager->initializeTrailingStop(entry_price, trail_percentage, PositionType::LONG);
    ...
    bool should_trigger = stop_loss_manager->shouldTriggerStop(trailing_stop, trigger_price, PositionType::LONG);
    EXPECT_TRUE(should_trigger);
    
    double safe_price = trailing_stop.stop_price + 0.01;
    bool should_not_trigger = stop_loss_manager->shouldTriggerStop(trailing_stop, safe_price, PositionType::LONG);
...
TEST_F(Phase4IntegrationTest, T4_5_1_IntegratedRiskSystem) {
    ...
    double stop_price = stop_loss_manager->calculateFixedStopLoss(entry_price, 0.05, PositionType::LONG);
...
TEST_F(Phase4IntegrationTest, T4_5_5_RiskSystemPerformance) {
    ...
    for (int i = 0; i < 100; ++i) {
        ...
        position.current_price = position.avg_cost * (0.9 + (rand() % 201) / 1000.0); // ±10% from cost
        position.market_value = position.shares * position.current_price;
        position.unrealized_pnl = position.market_value - (position.shares * position.avg_cost);
