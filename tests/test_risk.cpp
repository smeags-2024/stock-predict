...
TEST_F(RiskManagerTest, TrackingError) {
    ...
    auto te =
        stock_predict::RiskManager::calculate_tracking_error(sample_returns_, benchmark_returns);
...
TEST_F(RiskManagerTest, InformationRatio) {
    ...
    auto ir =
        stock_predict::RiskManager::calculate_information_ratio(sample_returns_, benchmark_returns);
...
TEST_F(RiskManagerTest, EmptyReturns) {
    ...
    EXPECT_THROW(stock_predict::RiskManager::calculate_var(empty_returns, 0.95), std::exception);
}
...
TEST_F(PositionSizerTest, FixedFractional) {
    ...
    auto position_size = stock_predict::PositionSizer::fixed_fractional(capital, risk_fraction,
...
TEST_F(PositionSizerTest, VolatilitySizing) {
    ...
    auto position_fraction =
        stock_predict::PositionSizer::volatility_sizing(capital, target_vol, asset_vol);
